...
def test_admin_userpermission_change_join_private_channels():
    ...
    with pytest.raises(AccessError):
        channel_join_v2(users['user2']['token'], channel_id1)
    ...
    channel_invite_v1(users['user2']['token'],channel_id1,users['user3']['auth_user_id'])

    # Global User 2 makes User 3 an owner of the channel
    channel_addowner_v1(users['user2']['token'],channel_id1,users['user3']['auth_user_id'])
