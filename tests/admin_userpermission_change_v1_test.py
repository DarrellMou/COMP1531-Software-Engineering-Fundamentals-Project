...
def test_admin_userpermission_change_invalid_token():
    ...
    with pytest.raises(AccessError):
        admin_userpermission_change_v1("Invalid owner", users['user1']['u_id'],2)
...
def test_admin_userpermission_change_invalid_owner():
    ...
    with pytest.raises(AccessError):
        admin_userpermission_change_v1(users['user2']['token'], users['user1']['u_id'],2)
...
def test_admin_userpermission_change_only_owner():
    ...
    with pytest.raises(InputError):
        admin_userpermission_change_v1(users['user1']['token'], users['user1']['u_id'],2)

# Checks basic test of changing a member into owner
def test_admin_userpermission_change_basic():
    users = setup_user()
    users['user1']['token']['permission_id'] == 1
    users['user2']['token']['permission_id'] == 2
    ...
    admin_userpermission_change_v1(users['user1']['token'], users['user2']['u_id'],1)

    assert users['user2']['token']['permission_id'] == 1
...
def test_admin_userpermission_change_join_private_channels():
    ...
    # User 1 is by default the global owner, and User 2 is a member
    assert users['user1']['token']['permission_id'] == 1
    assert users['user2']['token']['permission_id'] == 2
    assert users['user3']['token']['permission_id'] == 2

    # User 1 makes channel 1 and User 3 joins. User 1 sends a message
    channel_id1 = channels_create_v1(users['user1']['token'],'Test Channel',False)
    with pytest.raises(AccessError)
        channel_join_v1(users['user2']['token'], channel_id1)
    ...
    channel_invite_v1(users['user2']['token'],channel_id1,users['user3']['u_id'])

    # Global User 2 makes User 3 an owner of the channel
    channel_addowner_v1(users['user2']['token'],channel_id1,users['user3']['u_id'])
...
def test_admin_userpermission_change_ogowner():
    ...
    admin_userpermission_change_v1(users['user2']['token'],users['user1']['u_id'],2)
    assert users['user1']['token']['permission_id'] == 2

    # Global User 2 changes itself into member
    with pytest.raises(InputError):
        admin_userpermission_change_v1(users['user2']['token'], users['user2']['u_id'],2)
