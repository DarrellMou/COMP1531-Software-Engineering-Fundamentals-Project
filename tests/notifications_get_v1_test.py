...
def test_channelinvite_notif():
    reset_data()
    user1 = auth_register_v2('example1@hotmail.com', 'password1', 'first_name1', 'last_name1')
    user2 = auth_register_v2('example2@hotmail.com', 'password2', 'first_name2', 'last_name2')
    user3 = auth_register_v2('example3@hotmail.com', 'password3', 'first_name3', 'last_name3')

    chid1 = channels_create_v1(user1['auth_user_id'], 'Channel1', True)
    channel_invite_v1(user1['auth_user_id'], chid1['channel_id'], user2['auth_user_id'])
    channel_invite_v1(user2['auth_user_id'], chid1['channel_id'], user3['auth_user_id'])

    assert notifications_get_v1(user2) == {
        {
            'channel_id' : chid1,
            'dm_id' : -1,
    ...
    assert notifications_get_v1(user3['token']) == {
        {
            'channel_id' : chid1,
            'dm_id' : -1,
            'notification_message' : 'first_name2last_name2 added you to Channel1',
        },
    }
...
def test_channeltag_notif():
    reset_data()
    user1 = auth_register_v2('example1@hotmail.com', 'password1', 'first1', 'last1')
    user2 = auth_register_v2('example2@hotmail.com', 'password2', 'first2', 'last2')

    chid1 = channels_create_v1(user1['auth_user_id'], 'Channel1', True)
    channel_invite_v1(user1['auth_user_id'], chid1['channel_id'], user2['auth_user_id'])
    message_send_v2(user1, chid1, '@first2last2 1v1me')
    message_send_v2(user1, chid1, '@first2last2 1v1me but longer')
...
def test_dmcreate_notif():
    ...
    user1 = auth_register_v2('example1@hotmail.com', 'password1', 'first1', 'last1')
    user2 = auth_register_v2('example2@hotmail.com', 'password2', 'first2', 'last2')
    user3 = auth_register_v2('example3@hotmail.com', 'password3', 'first3', 'last3')

    add_user_list = {user2, user3}
    dmid1 = dm_create_v1(user1, add_user_list):
...
def test_dminvite_notif():
    ...
    add_user_list = {user2}
    dmid1 = dm_create_v1(user1['token'], add_user_list):
    dm_invite_v1(user2['token'], dmid1, user3['auth_user_id'])
...
def test_dmtag_notif():
    ...
    add_user_list = {user2}
    dmid1 = dm_create_v1(user1['token'], add_user_list):
    messid1 = message_senddm_v1(user1['token'], dmid1, '@first2last2 1v1me')
    messid2 = message_senddm_v1(user1['token'], dmid1, '@first2last2 1v1me but longer')
...
def test_notif_max():
    ...
    dmid1 = dm_create_v1(user1['token'], add_user_list):
    for x in range(0:22):
        message_senddm_v1(user1['token'], dmid1, '@first2last2 1v1me' + str(x))
