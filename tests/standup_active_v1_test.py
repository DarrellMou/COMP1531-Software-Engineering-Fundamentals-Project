...
def test_function_active(users):
    ...
    standup = standup_active_v1(users[0]['token'], ch_id0)
...
def test_multiple_runs(users):
    ...
    standup_start_v1(users[1]['token'], ch_id0['channel_id'], 3)
    standup = standup_active_v1(users[1]['token'], ch_id0)

    assert standup["is_active"] == True
    assert float("{:.2f}".format(standup["time_finish"])) == float("{:.2f}".format(datetime.now().timestamp() + 3))
    time.sleep(4)

    standup_start_v1(users[2]['token'], ch_id0['channel_id'], 5)
    standup = standup_active_v1(users[2]['token'], ch_id0)
...
def test_invalid_token(users):
    ...
    with pytest.raises(AccessError):
        standup_active_v1(12345, ch_id0)
