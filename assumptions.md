...
* Logout only ends the session that is related to the provided token, other sessions of the user remain valid. E.g. Token associated with a session on device A can be used to logout that session on device B.
