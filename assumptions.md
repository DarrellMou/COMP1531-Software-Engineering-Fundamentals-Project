...
* A user's first name, last name can be composed of any characters supported by python3, including symbols like '@#[]&^'.
...
* Logout only ends the session that is related to the provided token, other sessions of the user remain valid.
...
* A user's string handle can be composed of any characters supported by python3, including symbols like '@#[]&^'.

## users/all/v1 (winston)
* Users displayed in order of registration.
...
* The members appears the same order as they joined the channel.
...
* Given start will never be negative. 
* Start refers to the starting index of the data['dms'][dm_id]['messages'] list.
...
* The timestamp for our time_created key in our messages dictionary is based on UNIX time (epoch/POSIX time).
* Messages that are sent using message_send are appended to the message list within the channel.
...
* It is impossible for a user to 'remove' a message when there are no messages in the channel/dm (meaning nothing at all, not as in all messages are removed).
