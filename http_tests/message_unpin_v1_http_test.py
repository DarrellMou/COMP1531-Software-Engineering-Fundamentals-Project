...
def test_http_message_unpin_v1_InputError_unpin_removed_msg(set_up_data):
    setup = set_up_data
    user1, channel1 = setup['user1'], setup['channel1']
    ...
    response = requests.post(f"{url}message/unpin/v1", json=message_pin_body(user1, m_id["message_id"]))
    assert response.status_code == 400
